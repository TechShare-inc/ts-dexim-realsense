"""Shared config helpers for the dexim-realsense CLI."""
...
def get_realsense_config_dir(config_dir: str | None = None) -> Path:
    """Return the directory that stores named RealSense configs.

    Args:
        config_dir: Config root directory override.

    Returns:
        ``{config_dir}/dexim/realsense/`` as an absolute Path.
    """
    return get_config_dir(config_dir) / "dexim" / "realsense"


def get_config_yaml_path(name: str, config_dir: str | None = None) -> Path:
    """Return the canonical path for a named RealSense config.
    ...
    Returns:
        ``{realsense_config_dir}/{name}.yaml``.
    """
    return get_realsense_config_dir(config_dir) / f"{name}.yaml"
...
def resolve_config_path(name_or_path: str, config_dir: str | None = None) -> Path:
    ...
    2. ``{config_dir}/dexim/realsense/{name}.yaml``
    ...
    search_paths = [
        config_root / "dexim" / "realsense" / f"{name_or_path}.yaml",
...
def list_named_configs(config_dir: str | None = None) -> list[str]:
    """Return sorted logical names of all RealSense config YAMLs on disk.
    ...
    """
    realsense_dir = get_realsense_config_dir(config_dir)
    if not realsense_dir.is_dir():
        return []
    return sorted(p.stem for p in realsense_dir.glob("*.yaml"))
...
def create_config_yaml(
    ...
    """Create a new named RealSense config YAML.

    Args:
        name: Logical config name (e.g. ``lab-hw``).
...
def edit_config_yaml(
    ...
    """Patch fields in an existing named RealSense config YAML.
...
def remove_config_yaml(name: str, config_dir: str | None = None) -> Path:
    """Delete a named RealSense config YAML from disk.
