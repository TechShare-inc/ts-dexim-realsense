...
class RealSenseNode(PublisherDeviceNode):
    ...
    def get_status_info(self) -> StatusInfo:
        ...
        if (
            ...
            and getattr(self, "_teleop_active", False)
            and self.is_publishing
            and hasattr(self, "_rate_limiter")
            and self._rate_limiter is not None
        ):
            stats = self._rate_limiter.get_statistics()
            info.data_rate_hz = stats["actual_rate"]
            info.rate_limiter_stats = stats
        return info
